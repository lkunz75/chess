...
public class GameService {
    ...
    public ListResult list(ListRequest listRequest) throws DataAccessException {
        try {
            AuthData.AuthRecord authToken = service.registeredData.getAuthToken(listRequest.authToken());
            if (authToken == null) {
    ...
    public CreateResult create(CreateRequest createRequest) throws DataAccessException {
        try {
            AuthData.AuthRecord authToken = service.registeredData.getAuthToken(createRequest.authToken());
            if (authToken == null) {
                throw new DataAccessException("401 Error: Unauthorized");
            }
            else {
