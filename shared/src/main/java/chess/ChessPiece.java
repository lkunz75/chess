...
public class ChessPiece { // so confused by cloneable
    ...
    public List<List<Integer>> queenMovesCalculator(ChessBoard board, ChessPosition myPosition) {
        // can move diagonal, forward, backward, side to side so just call the ones that already do this
        List<List<Integer>> rook_moves = rookMovesCalculator(board, myPosition);
        List<List<Integer>> bishop_moves = bishopMovesCalculator(board, myPosition);
        rook_moves.addAll(bishop_moves);
        return rook_moves;
    }
